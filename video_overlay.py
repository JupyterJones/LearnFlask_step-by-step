...
@video_overlay_bp.route('/overlay_index', methods=['POST', 'GET'])
def overlay_index():
    return render_template('overlay_index.html')

@video_overlay_bp.route('/overlay_video', methods=['POST', 'GET'])
def overlay_video():
    if request.method == 'POST':
        try:
            ...
            filename=datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+".mp4"
            shutil.copy(video_path, 'static/assets/'+filename)
            ...
            ic(e)
            return redirect(url_for('video_overlay.show_result', video_path=output_video))            
            #return "An error occurred."

    return render_template('overlay_video.html')


@video_overlay_bp.route('/show_result', methods=['GET'])
def show_result():
